...
This file as published: `ar://kgBx7cLFHZn1FaAuc8IKns4ivL5NMWD81NTjOWbLf_Q`
That address is the **preceding** copy, not this one: a file cannot contain the
address of itself, because writing it would change what is being addressed. So
the line always lags by one publication, and every earlier copy stays on chain.
It said `620vnKvm…` — the August copy — through five republications on 20 Sep
before anyone looked at it.

Recording an address here at all is not circular: nothing hashes this file, so
...
**One figure in `PROVENANCE.md` is stale, and structurally will be again.** Its
table of pieces says the corpus is 107,114 files; it is 107,181, the difference
being the 67 Hindi files of *Light of the Bhāgavata* and *Life Comes from Life*
added on 20 Sep. It was not corrected, and the reason is worth stating:
`PROVENANCE.md` is *inside* the corpus manifest, so editing it moves the corpus
root, which moves the package root, which means recomputing on the machine with
the scans and republishing both manifests — about 0.4 credits to correct one
number in a descriptive sentence.

That file says of itself that it "carries no figure that changes, which is
precisely what lets it be certified along with the text it describes." That row
is the exception, and it is why the row keeps going stale. The fix is not to
update the number but to remove it: let the row point at this file, which is
outside every manifest and exists to carry the figures that move. Worth doing the
next time `PROVENANCE.md` is republished for a reason of its own.
